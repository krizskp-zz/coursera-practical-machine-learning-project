...
Randomly split the full training data (ptrain) into a smaller training set (ptrain1) and a validation set (ptrain2). This is to estimate the out-of-sample error.
...
Reduce the number of features by removing variables with nearly zero variance, variables that are almost always NA, and variables that don't make intuitive sense for prediction. Note that the features to remove are chosen by analyzing ptrain1, and performing the identical removals on ptrain2:
...
Start with a Random Forest model, to see if it would have acceptable performance.
Fit the model on ptrain1, and instruct the "train" function to use 3-fold cross-validation to select optimal tuning parameters for the model:
...
Use the fitted model to predict the label ("classe") in ptrain2, and show the confusion matrix to compare the predicted versus the actual labels:
